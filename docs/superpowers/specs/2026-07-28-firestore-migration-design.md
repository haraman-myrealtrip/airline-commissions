...
1. **라이브 시트 확정** — ⚠️ 로컬 `index.html`의 `SHEET_ID`(`1pmVuY...`)와 어제 조사 메모(`12qqW-...`)가 불일치. 이관 전 **현재 배포본이 실제로 읽는 시트가 무엇인지 반드시 확정**하고, 그 시트에서 최신값을 읽는다. (배포본 index.html 직접 확인 또는 사용자 확인)
2. 확정된 시트의 3개 탭을 읽어 위 컬렉션 구조로 Firestore에 1회 적재하는 스크립트 작성·실행.
